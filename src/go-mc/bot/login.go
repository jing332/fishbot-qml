...
func handleEncryptionRequest(c *Client, pack pk.Packet) error {
	...
	err := loginAuth(c.AsTk, c.Name, c.Auth.UUID, c.SessionURL, key, er) //向Mojang验证
	if err != nil {
...
func loginAuth(AsTk, name, UUID, sessionURL string, shareSecret []byte, er encryptionRequest) error {
	...
	PostRequest, err := http.NewRequest(http.MethodPost, sessionURL + "/session/minecraft/join",
