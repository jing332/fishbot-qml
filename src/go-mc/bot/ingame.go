...
func (c *Client) handlePacket(p pk.Packet) (disconnect bool, err error) {
	...
	case data.JoinGame:
		...
		if err == nil && c.Events.GameStart != nil {
			err = c.Events.GameStart(c.Address)
		}
...
func handleEntityMetadata(c *Client, p pk.Packet) error {
	if c.Events.EntityMetadataPacket != nil {
		err := c.Events.EntityMetadataPacket(p.Data)
		if err != nil {
			return err
		}
	}
...
func handleSpawnObject(c *Client, p pk.Packet) error {
	var (
		EID        pk.VarInt
		ObjUUID    pk.UUID
		Type       pk.VarInt
	...
	err := p.Scan(&EID, &ObjUUID, &Type, &X, &Y, &Z, &Pitch, &Yaw, &Data)
	if err != nil {
		return err
	}

	if int32(Type) == 106 && int(Data) == c.EntityID && c.Events.FishingHookIdUpdate != nil {
		err = c.Events.FishingHookIdUpdate(int32(EID))
		if err != nil {
			return err
		}
	}
...
func handleDisconnectPacket(c *Client, p pk.Packet) error {
	...
	if c.Events.Disconnect != nil {
		return c.Events.Disconnect(c.Address, reason)
	}
	return nil
}
...
func handleChatMessagePacket(c *Client, p pk.Packet) (err error) {
	...
	if c.Events.ChatMsg != nil {
		err = c.Events.ChatMsg(c.Address, string(s), byte(pos), string(sender.Encode()))
	}
